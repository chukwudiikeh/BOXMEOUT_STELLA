...
describe('MarketService.resolveMarket Unit Tests', () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(marketService as any, 'settlePredictions').mockResolvedValue(undefined);
    ...
    it('should throw error if market not found', async () => {
        ...
        await expect(marketService.resolveMarket('1', 1, 'source')).rejects.toThrow('Market not found');
    });

    it('should throw error if winning outcome is invalid', async () => {
        mockMarketRepository.findById.mockResolvedValue({ status: MarketStatus.CLOSED });
        await expect(marketService.resolveMarket('1', 2, 'source')).rejects.toThrow('Winning outcome must be 0 or 1');
    });

    it('should throw error if market is already RESOLVED', async () => {
        mockMarketRepository.findById.mockResolvedValue({ status: MarketStatus.RESOLVED });
        await expect(marketService.resolveMarket('1', 1, 'source')).rejects.toThrow('Market cannot be resolved in RESOLVED status');
    });

    it('should throw error if market is CANCELLED', async () => {
        mockMarketRepository.findById.mockResolvedValue({ status: MarketStatus.CANCELLED });
        await expect(marketService.resolveMarket('1', 1, 'source')).rejects.toThrow('Market cannot be resolved in CANCELLED status');
    });

    it('should throw error if market is DISPUTED', async () => {
        mockMarketRepository.findById.mockResolvedValue({ status: MarketStatus.DISPUTED });
        await expect(marketService.resolveMarket('1', 1, 'source')).rejects.toThrow('Market cannot be resolved in DISPUTED status');
    });
    ...
    it('should throw error if market is OPEN but closingAt has not passed', async () => {
        ...
        await expect(marketService.resolveMarket('1', 1, 'source')).rejects.toThrow('Market is still open and has not reached closing time');
    });

    it('should resolve market if status is CLOSED', async () => {
        mockMarketRepository.findById.mockResolvedValue({ status: MarketStatus.CLOSED });
        mockMarketRepository.updateMarketStatus.mockResolvedValue({ id: '1', status: MarketStatus.RESOLVED });
